...
// Task 2: constructors
vec::vec(double x_, double y_, double z_) : x(x_), y(y_), z(z_) {} // Chat GPT Instant 5.3
vec::vec() : vec(0,0,0) {} // Chat GPT Instant 5.3
// task 1: double values x, y, z.
void vec::set(double a, double b, double c){ // Chat GPT Instant 5.3
	x = a;
	y = b;
	z = c;
}
// Task 4: print function for debugging
void vec::print(const std::string& s) const{ // Chat GPT Instant 5.3
    std::cout << s << x << " " << y << " " << z << std::endl;
    } 
// Task 3: mathematical operators for 3D vectors
vec& vec::operator += (const vec& v) { // Chat GPT Instant 5.3
	x += v.x;
	y += v.y;
	z += v.z;
	return *this;
...
vec& vec::operator *= (double d) { // Chat GPT Instant 5.3
        x *= d;
        y *= d;
        z *= d;
        return *this;
...
double vec::dot(const vec& v) const{ // Chat GPT Instant 5.3
	return x*v.x + y*v.y + z*v.z;
}
vec vec::cross(const vec& v) const{ // Chat GPT Instant 5.3
	return vec(
		y*v.z - z*v.y,
		z*v.x - x*v.z,
		x*v.y - y*v.x
...
double vec::norm() const{ // Chat GPT Instant 5.3
	return std::sqrt(dot(*this));
}
// Also task 3: non-member operators
vec operator-(const vec& v){ // Chat GPT Instant 5.3
...
bool approx(double a, double b, double acc, double eps){ // Chat GPT Instant 5.3
